...
uv run -m main    # generate + upload character/event/disc wiki pages
```
...
- **Live2D talent images.** `tools/live2d_browser.py` loads
  `assets/assetbundles/actor2d/live2d_test.html`, which is gitignored and is not generated by
  any code in this repo. `page_generators/live2d_talent_images.py` — called from `main.py` —
  will not work until that file is copied over by hand.
